"""Pydantic v2 model layer for llm-schema events.

This module provides Pydantic v2 models that mirror the :class:`~llm_schema.event.Event`
...
core dependency.  Install it with::
    ...
    pip install "llm-schema[pydantic]"

Design goals
------------
* All field validation is equivalent to :meth:`~llm_schema.event.Event.validate`,
...
try:
    ...
    raise ImportError(
        "pydantic>=2.7 is required for llm_schema.models. "
        "Install it: pip install \"llm-schema[pydantic]\""
    ) from _import_err

from llm_schema.event import SCHEMA_VERSION, Event, Tags
from llm_schema.types import EVENT_TYPE_PATTERN
from llm_schema.ulid import validate as _validate_ulid

__all__ = ["EventModel", "TagsModel"]

# ---------------------------------------------------------------------------
# Validation patterns (must stay in sync with llm_schema/event.py)
# ---------------------------------------------------------------------------
...
class TagsModel(BaseModel):
    ...
    def from_tags(cls, tags: Tags) -> "TagsModel":
        """Construct from a :class:`~llm_schema.event.Tags` instance.

        Args:
            tags: A :class:`~llm_schema.event.Tags` instance.
    ...
    def to_tags(self) -> Tags:
        """Convert back to a :class:`~llm_schema.event.Tags` instance.

        Returns:
            A new :class:`~llm_schema.event.Tags` with the same key-value pairs.
        """
...
class EventModel(BaseModel):
    """Pydantic v2 model for the llm-schema event envelope.
    ...
    :meth:`~llm_schema.event.Event.validate`, so ``EventModel.from_event(event)``
    succeeds for any event that passes :meth:`~llm_schema.event.Event.validate`.
    ...
    signature: Optional[str] = Field(
        default=None,
        description="HMAC-SHA256 audit chain signature (set by llm_schema.signing).",
    )
    ...
    def from_event(cls, event: Event) -> "EventModel":
        """Construct an :class:`EventModel` from an :class:`~llm_schema.event.Event`.

        Args:
            event: A validated or unvalidated :class:`~llm_schema.event.Event`.
