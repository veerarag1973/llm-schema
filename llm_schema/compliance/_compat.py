"""Third-party adoption compatibility checker for llm-schema events.

This module provides the :func:`test_compatibility` function that applies the
llm-schema v1.0 compatibility checklist to a sequence of events.  The checks
...
CHK-2 — Event type is registered or valid custom
    If the event type matches a registered llm-schema namespace it passes.
...
from llm_schema.types import is_registered, validate_custom
...
def test_compatibility(events: Sequence[Event]) -> CompatibilityResult:
    """Apply the llm-schema v1.0 compatibility checklist to *events*.

    Args:
        events: One or more :class:`~llm_schema.event.Event` instances to
            inspect.
